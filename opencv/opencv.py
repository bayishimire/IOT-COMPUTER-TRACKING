...
from pyzbar.pyzbar import decode  # Import decode function for reading QR/barcodes
...
if not cap.isOpened():  # Check if the video stream opened successfully
    print("Cannot open ESP32-CAM stream")  # Print error if not
...
while True:  # Start an infinite loop to process video frames
    ...
    for obj in decoded_objects:  # Loop through all detected QR/barcodes
        ...
        for j in range(0, n):  # Draw lines between the points to form a box
            cv2.line(frame, hull[j], hull[(j + 1) % n], (255, 0, 0), 3)

        qr_data = obj.data.decode('utf-8')  # Decode the QR/barcode data as a string
        print(f"Decoded QR Code Data: {qr_data}")  # Print the decoded data
        ...
        cv2.putText(frame, qr_data, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, (0, 255, 0), 2)  # Display the decoded data on the frame

    cv2.imshow('ESP32-CAM QR Code Scanner', frame)  # Show the processed frame in a window
